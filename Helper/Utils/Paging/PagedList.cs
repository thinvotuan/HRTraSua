...
namespace BatDongSan.Utils.Paging
{
    public class PagedList<T> : List<T>, IPagedList<T>
    {
        ...
        public PagedList(IList<T> source, int index, int pageSize, bool custom, int? totalCount)
        {
            CustomInitialize(source, index, pageSize, totalCount);
        }
        ...
        protected void CustomInitialize(IEnumerable<T> source, int index, int pageSize, int? totalCount)
        {
            // for using it externally with 1 based page number
            index = index - 1;
